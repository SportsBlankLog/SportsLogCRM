rm -r /srv/tmp/dumps/*
docker exec -t srv_postgres_1 pg_dump -c -U dev SportsLogContext --format=p --encoding=UTF-8 --inserts | gzip > /srv/tmp/dumps/dump_SportsLog_`date +%Y-%m-%d"_"%H_%M_%S`.sql.gz
